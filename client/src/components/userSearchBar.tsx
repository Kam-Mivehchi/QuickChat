...
import { searchForUsers, createDM, addMemberToChat } from '../utils/api'
...
function UserSearchBar({ page }: { page: string }) {
   ...
   async function addMembers(userId: string) {
      try {
         ...
         let addMember = await addMemberToChat({
            params: { chatId: chatId },
            members: [userId],
         });


         dispatch({ type: ActionTypes.SET_CURRENT_CHAT, chatroom: addMember });
   ...
   return (
      <div className="">
         <span className="rounded-none w-full flex items-center relative " onClick={getUsers}>
            <input type="text" placeholder={`${page === "singleChat" ? "Add" : "Search"} By Username or Email`} className="input   border-none flex-grow " value={searchInput} onChange={(e) => setSearchInput(e.target.value)} />

            <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512" className='absolute right-5 top-1/2 -translate-y-1/2 fill-primary'><path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z" /></svg>
         </span>
         {/* dropdown */}

         <div className={` bg-base-100 relative transition-transform duration-200 ease-in ${openSearch ? "translate-x-0" : "translate-x-[100vw]"} z-40 mx-2  `}>
            <div className={` w-full  h-10 py-1 ${openSearch ? "block" : "hidden"}`} onClick={() => setOpenSearch(false)}>
            ...
            <div className="options overflow-auto max-h-[50vh] absolute z-40  w-full card  rounded-t-none
            justify-center items-between pt-24 lg:pt-48 bg-base-100 border-1 shadow-xl">
                  ...
                  matchingUsers.map((user: IUser) => {
                     ...
                     return (
                        <div className="flex items-center " key={`searchResponse${user._id}`}>
                           ...
                           <div className=" px-2">
                              {page === "singleChat" ?
                                 <button className={`btn btn-xs btn-secondary`} onClick={() => addMembers(user._id)}>+</button> : <label className="btn m-1 btn-ghost " onClick={() => handleNewDM(user._id)}>

                                    <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512" className='fill-primary'>
